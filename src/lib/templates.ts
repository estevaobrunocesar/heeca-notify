...
export function validateTemplate(name: string, bodyParams: string[], buttons: { type: string }[] = []): string | null {
  ...
  if (!spec) return /^heeca_(nail|lash|massage|brow|cut|dental|beauty|ink|piercing|skin|store|ticket|invoice|move)_/.test(name) ? null : `template ${name} não está no catálogo unificado`;
